...
class Shape_Generate():
    def __init__(self, cfgs):
        ...
        self.mode = 'step1'
        self.category = cfgs.get('seg_category', 'pspnet')
        self.cfgs = cfgs
        ...
        self.shape_prior=cfgs.get('shape_prior')
        # detailed parameters
        ...
        self.batchsize = cfgs.get('batchsize', 1)
    ...
    def init_netD_ellipsoid(self):
        if self.shape_prior == 'original':
            ellipsoid = self.init_shape_prior()
        ...
        elif self.shape_prior == 'smooth_box':
            ellipsoid = self.init_shape_prior()
            optimizer = torch.optim.Adam(
                filter(lambda p: p.requires_grad, self.netD.parameters()),
                lr=0.0001, betas=(0.9, 0.999), weight_decay=5e-4)
            print("Initializing the depth net to output smooth_box ...")
            for i in range(1000):
                depth_raw = self.netD(self.input_im)
                depth = depth_raw - depth_raw.view(1, 1, -1).mean(2).view(1, 1, 1, 1)
                depth = depth.tanh()
                depth = self.depth_rescaler(depth)
                ellipsoid=ellipsoid.detach().expand(depth.size(1),-1,-1)
                loss=F.mse_loss(depth[0],ellipsoid)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                if i % 100 == 0 and self.rank == 0:
                    print(f"Iter: {i}, Loss: {loss.item():.6f}")
        elif self.shape_prior == 'capsule':
            ellipsoid = self.init_shape_prior()
            optimizer = torch.optim.Adam(
                filter(lambda p: p.requires_grad, self.netD.parameters()),
                lr=0.0001, betas=(0.9, 0.999), weight_decay=5e-4)
            print("Initializing the depth net to output capsule ...")
            for i in range(1000):
                depth_raw = self.netD(self.input_im)
                depth = depth_raw - depth_raw.view(1, 1, -1).mean(2).view(1, 1, 1, 1)
                depth = depth.tanh()
    ...
    def init_shape_prior(self):
        with torch.no_grad():
            ...
            ##capsule
                ...
                torch.set_printoptions(threshold=torch.inf)
                max_y = max_y + (max_y - min_y) / 6
                # min_y = min_y - (max_y - min_y) / 12
                r = (max_y - min_y) / 2
                wide = (max_x - min_x) - r * 2
                capsule = torch.Tensor(1, h, w).fill_(far)
                y, x = torch.meshgrid(torch.linspace(0, w - 1, w), torch.linspace(0, h - 1, h))
                ratio = (max_y - min_y) / (max_x - min_x)
                y = (y - h / 2) / ratio + h / 2

                start = torch.tensor([min_x + r, min_y + r])
                end = torch.tensor([min_x + r + wide, min_y + r])
                dist = torch.zeros(self.image_size, self.image_size)
                for i in range(0, self.image_size):
                    for j in range(0, self.image_size):
                        s_point = torch.tensor([x[i, j], y[i, j]]) - start
                        s_e = end - start
                        t = torch.dot(s_point, s_e) / (torch.dot(s_e, s_e))
                        if t <= 0:
                            close_p_x = start[0]
                        elif t > 1:
                            close_p_x = end[0]
                        else:
                            close_p_x = start[0] + s_e[0] * t
                        close_p = torch.tensor([close_p_x, start[1]])
                        p_cp = close_p - torch.tensor([x[i, j], y[i, j]])
                        dist[i, j] = torch.sqrt((p_cp[0]) ** 2 + (p_cp[1]) ** 2)
                area = dist <= r
                depth = torch.sqrt(torch.abs(r ** 2 - dist ** 2))
                depth = near + ((r - depth) * (far - near)) / r
                capsule[0, area] = depth[area]
                shape_prior = capsule.cuda()


            ##smooth_box
            elif self.shape_prior == 'smooth_box':
                mask = self.input_mask[0].cpu()
                torch.set_printoptions(threshold=torch.inf)
                print(mask)
                noise = mask < 0.7
                mask[noise] = 0
                mask = (mask - 0.7) / (1 - 0.7)
                smooth_box = torch.Tensor(1, self.image_size, self.image_size).fill_(far)
                smooth_box = far - smooth_box * mask

                #smooth
                kernel_size = 11
                pad = 5
                n_convs=3
                conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=kernel_size, stride=1, padding=0)
                filt = torch.ones(1, 1, kernel_size, kernel_size)
                filt = filt / torch.norm(filt)
                conv.weight = torch.nn.Parameter(filt)
                smooth_box = smooth_box.unsqueeze(0)
                for _ in range(n_convs):
                    smooth_box = conv(smooth_box)
                    smooth_box = near + ((smooth_box - torch.min(smooth_box)) * (far - near)) \
                        / (torch.max(smooth_box) - torch.min(smooth_box))
                    smooth_box = torch.nn.functional.pad(smooth_box, tuple([pad] * 4), value=far)
                smooth_box=smooth_box.squeeze(0)
                shape_prior=smooth_box.cuda()

                ##box
            # elif self.shape_prior == 'box':
            #     center_x, center_y = int(self.image_size / 2), int(self.image_size / 2)
            #     box_height, box_width = int(self.image_size * 0.25), int(self.image_size * 0.25)
            #     box = torch.zeros(1, self.image_size, self.image_size)
            #     shape_prior = box[0,
    ...
    def init_parsing_model(self):
        if self.category == 'pspnet':
            ...
            from .parsing import VSNet
            classes = 4
            ckpt_path = 'checkpoints/parsing/VSNet.pth'
            self.parse_model = VSNet(classes=classes, pretrained=False)
            temp = nn.DataParallel(self.parse_model)
            checkpoint = torch.load(ckpt_path, map_location=map_func)
            temp.load_state_dict(checkpoint['state_dict'], strict=False)
            self.parse_model = temp.module
    ...
    def forward_step1(self):
        ...
        self.light_a = self.light[:,:1] /2+0.5
        self.light_b = self.light[:,1:2] /2+0.5
